...
const PopupMovie = ({ open, onClose, onComfirm, formState, handleChange, handleOnClose }) => {

	return (
		<Modal isOpen={open} toggle={handleOnClose} id='exampleModalCenter' style={{width: '50%', maxWidth: 'unset'}}>
			<ModalHeader toggle={handleOnClose}>{!formState.id ? 'Tạo phim' : 'Sửa phim'}</ModalHeader>
				...
				<Form>
					...
					<FormGroup>
						...
						<Input
							type='text'
							name='premiereDate'
							value={formState.premiereDate === undefined ? "" : formState.premiereDate}
							onChange={(event) => handleChange(event)}
						/>
					</FormGroup>
					<FormGroup>
					...
					<FormGroup>
						...
						<Input
							type='text'
							name='imageInfo'
							value={formState.imageInfo === undefined ? "" : formState.imageInfo}
							onChange={(event) => handleChange(event)}
						/>
					</FormGroup>
					<FormGroup>
